...
def create_app():
	...
	def clean():
		...
		stale = mongo_db.data.find({'time': {'$lt': datetime.today() - timedelta(days=1)}})
		mongo_db.data.delete_many({'time': {'$lt': datetime.today() - timedelta(days=1)}})

		for i in stale:
			stale_session_id = i['_id']
			db_resource_deleter(f"data/{stale_session_id}/certificate", resource_type="image")
			db_resource_deleter(f"data/{stale_session_id}/sheet", resource_type="raw")
